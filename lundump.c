/*
** $Id: lundump.c,v 1.33 2000/10/31 16:57:23 lhf Exp lhf $
...
static TString* LoadString (lua_State* L, ZIO* Z, int swap)
{
 ...
 {
  char* s=luaO_openspace(L,size,l_char);
...
static Proto* LoadFunction (lua_State* L, ZIO* Z, int swap)
{
 ...
 if(0)
 if (luaG_checkcode(L,tf)==0) luaO_verror(L,"bad code in `%.99s'",ZNAME(Z));
 return tf;
}
